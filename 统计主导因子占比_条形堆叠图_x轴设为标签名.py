    ...
    for tif_file in tqdm(tif_files[:max_files], desc="Processing TIFF files"):
        try:
            with rasterio.open(tif_file) as src:
                ...
                # 按像素值排序
                sorted_percentages = sorted(percentages.items(), key=lambda x: x[0])
                ...
                file_data.append({
                    ...
                    "pixel_values": [x[0] for x in sorted_percentages]
                })
...
def create_stacked_bar_chart(file_data, output_folder):
    ...
    pixel_values = sorted({pv for data in file_data for pv in data["pixel_values"]})

    # 创建颜色映射
    colors = [color_dict[pv] for pv in pixel_values]
    ...
    for i, pv in enumerate(pixel_values):
        ...
        for data in file_data:
            percentages.append(data["percentages"].get(pv, 0))

        ax.barh(file_names, percentages, left=bottom, color=colors[i], label=labels_dict.get(pv, str(pv)))
        bottom += np.array(percentages)

    # 设置图表属性
    ax.set_xlabel('Percentage (%)', fontsize=12)
    ax.set_ylabel('Image Files', fontsize=12)
    ax.set_title('Dominant Drivers Distribution (Stacked)', fontsize=14, pad=20, weight='bold')

    # 添加图例（放在图表外部右侧）
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0., title="Variables")
    ...
    # 保存PNG
    output_png = os.path.join(output_folder, "stacked_bar_chart.png")
    plt.savefig(output_png, dpi=900, bbox_inches='tight', transparent=False)
    plt.close()

    print(f"Successfully saved stacked bar chart to: {output_png}")
    return output_png
