...
def evaluate_models(data_path, target_column,
    ...
    for i, seed in enumerate(seed_range[:n_repeats]):
        ...
        ax1.bar(x[0] - width/2, new['R²_mean'], width, yerr=new['R²_std'], color='#BFA930', capsize=5)
        ax1.bar(x[0] + width/2, old['R²_mean'], width, yerr=old['R²_std'], color='#240672', capsize=5)
        ax1.set_ylabel('R²')
        # ax1.set_ylim(0, 1)

        ax1b = ax1.twinx()
        ax1b.bar(x[1] - width/2, new['RMSE_mean'], width, yerr=new['RMSE_std'], color='#BFA930', capsize=5)
        ax1b.bar(x[1] + width/2, old['RMSE_mean'], width, yerr=old['RMSE_std'], color='#240672', capsize=5)
        ax1b.set_ylabel('RMSE')

        ax1.set_xticks(x)
        ax1.set_xticklabels(['R²', 'RMSE'])
        ax1.set_title('Maize NOEF: R² and RMSE')
        ...
        ax2.scatter(x_all, y_all_new, alpha=0.3, c='#BFA930', edgecolors='none', s=20)
        ax2.scatter(x_all, y_all_old, alpha=0.3, c='#240672', edgecolors='none', s=20, marker='x')
        ...
        def plot_all_regressions_with_mean(x, y, ax, color, model_name):
            ...
            ax.plot(x_vals, y_mean, color=color, linewidth=2, zorder=3,
                    label=f'{model_name} Mean Fit')

        plot_all_regressions_with_mean(x_all, y_all_new, ax2, '#BFA930', 'New Model')
        plot_all_regressions_with_mean(x_all, y_all_old, ax2, '#240672', 'Old Model')

        ax2.plot([plot_min, plot_max], [plot_min, plot_max], 'k--', alpha=0.5, zorder=0, label='1:1 Line')
        ax2.set_xlabel('Actual')
        ax2.set_ylabel('Predicted')
        ax2.set_title(f'Maize NOEF: Predicted vs Actual (100 samples)')
        # ax2.legend()

        plot_path = os.path.join(output_dir, f'Maize_NOEF_comparison_plot_{timestamp}.png')
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.close()

        results_df.to_csv(os.path.join(output_dir, f'Maize_NOEF_comparison_metrics_{timestamp}.csv'), index=False)
        summary_stats.to_csv(os.path.join(output_dir, f'Maize_NOEF_summary_stats_{timestamp}.csv'), index=False)
        all_validation_data_df.to_csv(os.path.join(output_dir, f'Maize_NOEF_validation_predictions_{timestamp}.csv'),
                                      index=False)
        metrics_path = os.path.join(output_dir, f'Maize_NOEF_comparison_metrics_{timestamp}.csv')
        summary_path = os.path.join(output_dir, f'Maize_NOEF_summary_stats_{timestamp}.csv')
        validation_path = os.path.join(output_dir, f'Maize_NOEF_validation_predictions_{timestamp}.csv')
...
if __name__ == "__main__":
    try:
        ...
        # 模型1信息
        # model1_path = r'D:\CNPK\N_CODE_PY\N_ML\RF_model_C_new_results0727_GLO_所有变量特征\Wheat\N2OEF\model.joblib' # √√
        ...
        # model1_path = r'D:\CNPK\N_CODE_PY\N_ML\RF_model_C_new_results0727_GLO_所有变量特征\Wheat\NOEF\model.joblib' # √√
        # model1_path = r'D:\CNPK\N_CODE_PY\N_ML\RF_model_C_new_results0727_GLO_所有变量特征\Wheat\LF\model.joblib' # √√
        # model1_path = r'D:\CNPK\N_CODE_PY\N_ML\RF_model_C_new_results0727_GLO_所有变量特征\Wheat\RF\model.joblib' #
        # model1_path = r'D:\CNPK\N_CODE_PY\N_ML\RF_model_C_new_results0727_GLO_所有变量特征\Maize\RF\model.joblib' # √√
        # model1_path = r'D:\CNPK\N_CODE_PY\N_ML\RF_model_C_new_results0727_GLO_所有变量特征_去掉异常值\Wheat\RF\model.joblib' # √√
        # model1_path = r'D:\CNPK\N_CODE_PY\N_ML\RF_model_C_new_results0727_GLO_所有变量特征\Wheat\NUE\model.joblib' # √√
        # model1_path = r'D:\CNPK\N_CODE_PY\N_ML\RF_model_C_new_results0727_GLO_所有变量特征\Maize\N2OEF\model.joblib' # √√
        model1_path = r'D:\CNPK\N_CODE_PY\N_ML\RF_model_C_new_results0727_GLO_所有变量特征_去掉异常值_TEST\Maize\NOEF\model.joblib' # √√
        # model1_path = r'D:\CNPK\N_CODE_PY\N_ML\RF_model_C_new_results0727_GLO_所有变量特征_去掉异常值\Maize\LF\model.joblib' # √√
        # model1_path = r'D:\CNPK\N_CODE_PY\N_ML\RF_model_C_new_results0727_GLO_所有变量特征_去掉异常值\Maize\RF\model.joblib'
        # model1_path = r'D:\CNPK\N_CODE_PY\N_ML\RF_model_C_new_results0727_GLO_所有变量特征_去掉异常值\Maize\NUE\model.joblib' # √√
        ...
        model1_features = [
            ...
            # 'Cddw_Key_mean',
            'Hddm_Span_max', 'Hddm_Span_min', 'Hddm_Key_mean',
            'Cddm_Span_max', 'Cddm_Span_min', 'Cddm_Key_mean'
        ]

        model2_path = r'D:\CNPK\N_CODE_PY\N_ML\RF_model_C_old_results0612_GLO\Maize\NOEF\model.joblib'
        ...
        results_df, summary_stats, plot_path, metrics_path, summary_path, validation_path = evaluate_models(
            ...
            output_dir='Maize_model_comparison_results0727',
            n_repeats=100,
