...
def process_nc_files(excel_path, nc_folder, output_path):
    ...
    df[[f"SMrz_Month_{i}" for i in range(1, 13)]] = None  # 初始化存储列
    ...
    for year in range(1982, 2021):
        # 构建NC文件路径
        nc_file = f"SMrz_{year}_GLEAM_v4.2a_MO.nc"
        ...
        with xr.open_dataset(nc_file) as ds:
            ...
            # 更新DataFrame
            df.loc[year_df.index, [f"SMrz_Month_{i}" for i in range(1, 13)]] = monthly_values
...
if __name__ == "__main__":
    # 配置路径
    excel_file = Path("Dataset_upd_ro_sro_slp_LAI_fd_rx1_rx5_ddp_spei_Hddw_Cddw_phaseggcmi_mon_Hddm_Cddm_sm_yield.xlsx")
    nc_folder = Path(r"D:\CNPK\Nloss_DATA\GLEAM4_SMrz_month\SMrz\SMrz")
    output_file = Path("Dataset_upd_ro_sro_slp_LAI_fd_rx1_rx5_ddp_spei_Hddw_Cddw_phaseggcmi_mon_Hddm_Cddm_SMs_yield_SMrz.xlsx")
