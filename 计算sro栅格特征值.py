...
def get_target_profile(nc_file):
    ...
    with xr.open_dataset(nc_file) as ds:
        latitude = np.sort(ds.latitude.values)
        longitude = np.sort(ds.longitude.values)
...
def process_sro_data(nc_file, target_profile):
    """处理sro数据（无缩放）"""
    with xr.open_dataset(nc_file) as ds:
        times = pd.to_datetime(ds.valid_time.values)
        time_mask = (times.year == 2018)  | (times.year == 2019)

        sro_data = ds.sro.sel(valid_time=time_mask).load()

        # 维度处理
        if 'scale' in sro_data.dims:
            sro_data = sro_data.isel(scale=0)  # 选择第一个scale
        sro_data = sro_data.squeeze(drop=True)

        # 确保维度顺序
        if set(sro_data.dims) == {'time', 'lat', 'lon'}:
            sro_data = sro_data.transpose('time', 'lat', 'lon')

        return sro_data.astype(np.float32), times[time_mask]


def calculate_features(plant, harvest, sro_data, times):
    """特征计算（无缩放）"""
...
def save_results(data_dict, profile):
    ...
    for suffix, data in data_dict.items():
        with rasterio.open(f"sro_Span_{suffix}.tif", 'w', **output_profile) as dst:
            dst.write(data.astype(np.float32), 1)


if __name__ == "__main__":
    NC_FILE = "ERA5_2014_2020.nc"
    PLANT_PATH = "Maize_plant_0.083.tif"
    HARVEST_PATH = "Maize_harvest_0.083.tif"
    ...
    sro_data, times = process_sro_data(NC_FILE, target_profile)
    print(f"处理时间段: {times[0]} 至 {times[-1]}")

    max_val, min_val, mean_val = calculate_features(plant, harvest, sro_data, times)
