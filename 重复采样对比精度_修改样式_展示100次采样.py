...
def evaluate_models(data_path, target_column,
    ...
    for i, seed in enumerate(seed_range[:n_repeats]):
        ...
        ax1.bar(x[0] - width/2, new['R²_mean'], width, yerr=new['R²_std'], color='#41046F', capsize=5)
        ax1.bar(x[0] + width/2, old['R²_mean'], width, yerr=old['R²_std'], color='#A668D5', capsize=5)
        ax1.set_ylabel('R²')
        ax1.set_ylim(0, 1)

        ax1b = ax1.twinx()
        ax1b.bar(x[1] - width/2, new['RMSE_mean'], width, yerr=new['RMSE_std'], color='#41046F', capsize=5)
        ax1b.bar(x[1] + width/2, old['RMSE_mean'], width, yerr=old['RMSE_std'], color='#A668D5', capsize=5)
        ax1b.set_ylabel('RMSE')

        ax1.set_xticks(x)
        ax1.set_xticklabels(['R²', 'RMSE'])
        ax1.set_title('Wheat N2OEF: R² and RMSE')
# ---------- 右侧散点图（100次） ----------
        ...
        ax2.scatter(x_all, y_all_new, alpha=0.3, c='#41046F', edgecolors='none', s=20)
        ax2.scatter(x_all, y_all_old, alpha=0.3, c='#A668D5', edgecolors='none', s=20)

        def plot_regression_with_ci_mean_line(x, y, ax, color):
            slope = results_df[results_df['Model'] == ('New Model' if color == '#41046F' else 'Old Model')]['Slope'].mean()
            intercept = results_df[results_df['Model'] == ('New Model' if color == '#A668D5' else 'Old Model')][
                'Intercept'].mean()
            ...
            confidence = 0.90
            alpha = 1 - confidence
            t_val = stats.t.ppf(1 - alpha / 2, dof)  # 90%置信区间
            ci = t_val * residual_std * np.sqrt(1 / len(x) + (x_vals - np.mean(x)) ** 2 / np.sum((x - np.mean(x)) ** 2))
            ax.fill_between(x_vals, y_vals - ci, y_vals + ci, color='red', alpha=0.5)

        plot_regression_with_ci_mean_line(x_all, y_all_new, ax2, '#41046F')
        plot_regression_with_ci_mean_line(x_all, y_all_old, ax2, '#A668D5')

        ax2.plot([plot_min, plot_max], [plot_min, plot_max], 'k--', alpha=0.5)
        ax2.set_xlabel('Actual')
        ax2.set_ylabel('Predicted')
        ax2.set_title(f'Wheat N2OEF: Predicted vs Actual (100 samples)')

        plot_path = os.path.join(output_dir, f'Wheat_N2OEF_comparison_plot_{timestamp}.png')
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.close()

        results_df.to_csv(os.path.join(output_dir, f'Wheat_N2OEF_comparison_metrics_{timestamp}.csv'), index=False)
        summary_stats.to_csv(os.path.join(output_dir, f'Wheat_N2OEF_summary_stats_{timestamp}.csv'), index=False)
        all_validation_data_df.to_csv(os.path.join(output_dir, f'Wheat_N2OEF_validation_predictions_{timestamp}.csv'),
                                      index=False)
        metrics_path = os.path.join(output_dir, f'Wheat_N2OEF_comparison_metrics_{timestamp}.csv')
        summary_path = os.path.join(output_dir, f'Wheat_N2OEF_summary_stats_{timestamp}.csv')
        validation_path = os.path.join(output_dir, f'Wheat_N2OEF_validation_predictions_{timestamp}.csv')
...
if __name__ == "__main__":
    try:
        ...
        model2_path = r'D:\CNPK\N_CODE_PY\N_ML\RF_model_C_old_results0612_GLO\Wheat\N2OEF\model.joblib'
