...
class CropSpecificModel:
    ...
    def _clean_data(self, data, target):
        ...
        # 新增：过滤异常值（3个标准差）
        if target in ['N2OEF']:  # 可以根据需要添加其他目标变量
        ...
        if target in ['NH3EF']:  # 可以根据需要添加其他目标变量
            print(f"过滤前样本量: {len(df_clean)}")
        ...
        if target in ['NOEF']:  # 可以根据需要添加其他目标变量
            print(f"过滤前样本量: {len(df_clean)}")
        ...
        if target in ['LF']:  # 可以根据需要添加其他目标变量
            print(f"过滤前样本量: {len(df_clean)}")
        ...
        if target in ['RF']:  # 可以根据需要添加其他目标变量
            print(f"过滤前样本量: {len(df_clean)}")
        ...
        if target in ['NUE']:  # 可以根据需要添加其他目标变量
            print(f"过滤前样本量: {len(df_clean)}")
    ...
    def save_results(self, output_dir):
        ...
        for target, result in self.results.items():
            ...
            # 保存模型
            # joblib.dump(result['model'], os.path.join(target_dir, 'model.pkl'))
            # 修改后（保存为.joblib）
...
if __name__ == "__main__":
    # 初始化输出目录
    output_dir = "RF_model_C_new_results0727_GLO_所有变量特征_去掉异常值_TEST"
    ...
    data = load_and_preprocess(
        'Dataset_upd_ro_sro_slp_LAI_fd_rx1_rx5_ddp_spei_Hddw_Cddw_phaseggcmi_mon_Hddm_Cddm_sm_yield.csv')
